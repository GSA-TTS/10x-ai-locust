...
class WebSocketClient:
        ...
        while self.is_open and self.conn:
            try:
                ...
                if '42["message"' in message and "assistant" in message:
                    print(
                        f"Assistant response message detected, signaling response event"
                    )
    ...
    def send_chat_message(self, payload, name=None):
        ...
        try:
            ...
            print(
                f"SENDING: {message[:100]}..."
                if len(message) > 100
                else f"SENDING: {message}"
            )
            ...
            if self.response_event.wait(timeout=60):  # 60-second timeout
                ...
                print(
                    f"Response event triggered, processing {len(self.messages)} messages in queue"
                )
                for msg in list(self.messages):
                ...
                print(
                    f"Timeout reached. Current message queue has {len(self.messages)} messages"
                )
                # print first few messages in queue for debugging
                for i, msg in enumerate(self.messages[:3]):
                    print(
                        f"Queue message {i}: {msg[:100]}..."
                        if len(msg) > 100
                        else f"Queue message {i}: {msg}"
                    )
...
class WebSocketChatUser(WebSocketUser):
    ...
    def chat_completion(self):
        ...
        success, response_text = self.ws_client.send_chat_message(
            message_payload, "/ws/chat/completions"
        )
        ...
        if success:
            ...
            try:
                ...
                if "content" in response_data:
                    ...
                    if (
                        content_validation_string
                        and content_validation_string.lower() in complete_text.lower()
                    ):
                ...
                tokens_per_second = (
                    num_output_tokens / (response_time / 1000)
                    if response_time > 0
                    else 0
                )
                ...
                if content_validated:
                    ...
                    print(
                        f"Content validation failed for text: ...{complete_text[:100]}"
                    )
...
if __name__ == "__main__":
    run_single_user(WebSocketChatUser)
